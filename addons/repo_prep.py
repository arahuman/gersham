...
# check if repo-prep.py is being run standalone or called from another
# python file
...
# set the repository's root folder here, if the script user has not set a
# custom path.
if standalone:
    if repo_root == False:
        repo_root = os.getcwd()
...
class Compressor:
    ...
    def __init__(self):
        ...
        self.addon_zip_output = "downloads"
    ...
    def master(self):
        ...
        for addon in mydir:
            ...
            print "Add on path is " + self.addon_path
            # skip any file or .svn folder.
            if is_addon_dir(self.addon_path):
                ...
                addon_zip_exists = self._get_zipped_addon_path(self.addon_version_number)

                # generator class relies on addon.xml being in release folder.
                # so if need be, fix a zipped addon release folder lacking an
                # addon.xml
                if addon_zip_exists:
                    print "Addon Zip exists"
                else:
                    print "No addon zip exists"
                    if addon_xml_exists:
                        # now addon.xml has been read, scrape version number
                        # from it. we need this when naming the zip (and if it
                        # exists the changelog)
                        self._read_version_number()
    ...
    def _get_zipped_addon_path(self,revision_number):
        # get name of addon zip file. returns False if not found.
        for the_file in self.addon_folder_contents:
            if '.zip' in the_file:
                if (self.addon_name + '-'+revision_number) in the_file:
                    self.addon_zip_path = os.path.join(
                        self.addon_path, the_file)
                    return True
        # if loop is not broken by returning the addon path, zip was not found
        # so return False
        self.addon_zip_path = None
        return False
    ...
    def _extract_addon_xml_to_release_folder():
        ...
        print "extract addon xml release folder " + the_zip
    ...
    def _recursive_zipper(self, dir, zip_file):
        ...
        for root, dirs, files in os.walk(dir):
            ...
            # subtract the source file's root from the archive root - ie. make
            # /Users/me/desktop/zipme.txt into just /zipme.txt
            archive_root = os.path.abspath(root)[root_len:]

            for f in files:
                if '.zip' in f:
                    print "Skipping the zip file"
                else:
    ...
    def _create_compressed_addon_release(self):
        # create a zip of the addon into repo root directory, tagging it with
        # '-x.x.x' release number scraped from addon.xml
        ...
        # now move the zip into the addon folder, which we will now treat as
        # the 'addon release directory'
        os.rename(zippath, os.path.join(self.addon_path, zipname))

        # in the addon release directory, delete every file apart from
        # addon.xml, changelog, fanart, icon and the zip we just constructed.
        # also rename changelog.
        ##for the_file in self.addon_folder_contents:
            ...
            # list of files we specifically need to retain for the addon
            # release folder (folder containing the zip
            ...
            # hangelog so that it is detected irrespective of whether C is
            # capitalised
            ##elif 'hangelog' in the_file:
    ...
    def _read_version_number(self):
        ...
        headers = re.compile(
            "\<addon id\=(.+?)>", re.DOTALL).findall(self.addon_xml)

        for header in headers:

            # if this is the header for the addon, proceed
            if self.addon_name in header:
                # clean line of quotation characters so that it is easier to
                # read.
                ...
                self.addon_version_number = (
                    (re.compile("version\=(.+?) ", re.DOTALL).findall(header))[0]).strip()

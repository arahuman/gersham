...
class PrayTimes():
    ...
    def __init__(self, method = "MWL") :
        ...
        for method1, config in self.methods.items():
...
if __name__ == "__main__":
    ...
    times = prayTimes.getTimes(date.today(), (35.756, -86.885), -5,0,'24h');
    print "Prayer Method is " + str(prayTimes.getMethod())
    for i in ['Imsak','Fajr', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib','Sunset', 'Isha', 'Midnight']:
        print(i+ ': '+ str(times[i.lower()]))

    testdate = [datetime.datetime(2012,07,04,1,34,00),
                datetime.datetime(2012,07,04,2,34,00),
                datetime.datetime(2012,07,04,3,34,00),
                datetime.datetime(2012,07,04,4,0,00),
                datetime.datetime(2012,07,04,4,9,00),
                datetime.datetime(2012,07,04,4,10,00),
                datetime.datetime(2012,07,04,5,37,00),
                datetime.datetime(2012,07,04,12,51,00),
                datetime.datetime(2012,07,04,12,52,00),
                datetime.datetime(2012,07,04,16,42,00),
                datetime.datetime(2012,07,04,20,7,00),
                datetime.datetime(2012,07,04,21,35,00),
                datetime.datetime(2012,07,04,23,59,00)
                ]    

    for now in testdate :
        times = prayTimes.getTimes(now.date(), (35.756, -86.885), -5,0,'24h');
        print ('='*41)
        print "Test Date is %s" % str(now)
        #print "Prayer Method is " + str(prayTimes.getMethod())
        ...
        for i in range(len(schNames)):
                ...
                #print ("%d%02d%02d " % (now.year,now.month,now.day))+ str(times[i.lower()])
            firstPrayTime = datetime.datetime.strptime(("%d%02d%02d " % (now.year,now.month,now.day))+ str(times[schNames[i].lower()]),"%Y%m%d %H:%M")
            if i<len(schNames)-1:
                nextPrayTime = datetime.datetime.strptime(("%d%02d%02d " % (now.year,now.month,now.day))+ str(times[schNames[i+1].lower()]),"%Y%m%d %H:%M")
            else :
                #nextPrayTime = now + datetime.timedelta(days=1)
                nextPrayTime = datetime.datetime.strptime(("%d%02d%02d " % (now.year,now.month,now.day))+ str(times[schNames[1].lower()]),"%Y%m%d %H:%M")
                nextPrayTime = nextPrayTime + datetime.timedelta(days=1);
